...
CONSOLE_FORMAT = [
    ...
    ("mpc/goals_reached", "G", "float"),
    ("pi/goals_reached", "Gpi", "float"),
    ("total_time", "T", "time"),
]

# Extra eval.csv columns, named as in ../comfree_tdmpc's eval.csv. Written only when
# the eval metrics contain them (comfree-allegro-cube); other tasks are unchanged.
EXTRA_EVAL_CSV_KEYS = [
    f"{p}/{k}"
    for p in ("pi", "mpc")
    for k in ("return", "return_std", "goals_reached", "dropped", "length")
]
CAT_TO_COLOR = {
    "pretrain": "yellow",
    "train": "blue",
...
class Logger:
    ...
    def __init__(self, cfg):
        ...
        wandb.init(
            ...
            entity=self.entity,
            name=f"{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_{cfg.task}.tdmpc.{cfg.exp_name}.{cfg.seed}",
            #group=self._group,
    ...
    def log_comfree_train(self, row):
        """Append a comfree_tdmpc-style row to <work_dir>/train.csv."""
        if not self._save_csv:
            return
        self._comfree_train.append(row)
        pd.DataFrame(self._comfree_train).to_csv(self._log_dir / "train.csv", index=None)
    ...
    def log(self, d, category="train"):
        ...
        if category == "eval" and self._save_csv:
            ...
            if "mpc/goals_reached" in d:  # comfree-allegro-cube only
                keys += [k for k in EXTRA_EVAL_CSV_KEYS if k in d]
